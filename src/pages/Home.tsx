...
import { Github, Twitter, Linkedin, ArrowUpRight, Music, Send } from 'lucide-react';
import {
  SiTypescript, SiJavascript, SiCplusplus, SiPython, SiRemix, SiVite,
  SiNextdotjs, SiPrisma, SiUnrealengine, SiArchlinux, SiNodedotjs,
  SiPostgresql, SiGraphql, SiGo, SiRust, SiFirebase, SiDocker, SiCloudflare
} from 'react-icons/si';
  ...
  useEffect(() => {
    const fetchMusic = async () => {
      try {
        ...
        const res = await fetch(`https://www.googleapis.com/youtube/v3/search?part=snippet&type=video&q=ice+slowed+zertal&key=${apiKey}`);
        const data = await res.json();
